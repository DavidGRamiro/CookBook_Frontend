...
export class VerUnaComponent implements OnInit {
          ...
          .subscribe(
            (data) => {
              ...
              this._msg.add({
                severity: 'info',
                summary: '¡ Gracias !',
                detail: 'Tu comentario ayuda a seguir mejorando',
              });
  ...
  eliminarComentario(idComentario: number) {
    if (idComentario != null && idComentario != undefined) {
      this._userService.eliminarComentario(idComentario).subscribe(
        (data) => {
          this._msg.add({
            severity: 'info',
            summary: 'El comentario ha sido eliminado',
          });
        },
        (error) => {
          this._msg.add({
            severity: 'error',
            summary: 'No se ha podido eliminar el comentario',
          });
        }
      );
    } else {
      this._msg.add({
        severity: 'warning',
        summary: 'No existe el comentario',
      });
    }
  }
}
